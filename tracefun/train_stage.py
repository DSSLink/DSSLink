...
def main(datasets_dir, outputs_dir, dname, group, lab_frac, epochs, use_new_links):
    sys.argv.append(f'--data_dir=/home/zhu/projects5/datasets/{dname}/{group}')
    sys.argv.append(f'--output_dir=/home/zhu/projects5/outputs/{dname}/{group}')
    sys.argv.append('--model_path=/home/zhu/projects/siamese_model')
    sys.argv.append('--code_bert=/home/zhu/projects/codeBERT')
    sys.argv.append(f'--exp_name=model_lab{lab_frac}_epochs{epochs}')
    ...
    train_lab_examples = load_examples(os.path.join(args.data_dir, f'train_lab{lab_frac}'), model, use_new_links=use_new_links)
    valid_examples = load_examples(os.path.join(args.data_dir, 'valid'), model)
    train(args, train_lab_examples, valid_examples, model)
...
def train(args, train_lab_examples, valid_examples, model):
    """
    :param args:
    ...
    example_num = 2 * len(train_lab_examples)  # 50/50 of pos/neg
    ...
    if args.max_steps > 0:
        ...
        if epoch_batch_num < args.gradient_accumulation_steps:
            args.gradient_accumulation_steps = epoch_batch_num
    ...
    for epoch in train_iterator:
        train_with_neg_sampling(args, model, train_lab_examples, optimizer, scheduler, tb_writer, step_bar, skip_n_steps_in_epoch, criterion)
...
def train_with_neg_sampling(args, model, train_lab_examples, optimizer, scheduler, tb_writer, step_bar, skip_n_steps, criterion):
    """
    Create training dataset at epoch level.
    """
    model.train()
    tr_loss, tr_ac = 0, 0
    train_dataloader = train_lab_examples.random_neg_sampling_dataloader(batch_size=args.batch_size)
    for step, batch in enumerate(train_dataloader):
        if skip_n_steps > 0:
            skip_n_steps -= 1
            continue

        inputs = format_batch_input(batch, train_lab_examples, model)
