...
from common.data import load_unlabeled_data
...
def main(datasets_dir, outputs_dir, dname, group, lab_dtype, unlab_dtype, epochs, task_type=None):
    ...
    if task_type is None:
        sys.argv.append(f'--exp_name=model_lab{lab_dtype}_lp_epochs{epochs}')
    else:
        sys.argv.append(f'--exp_name=model_lab{lab_dtype}_lp_{task_type}_epochs{epochs}')
    ...
    train_loader_noshuff, train_dataset, unlab_examples_num = load_unlabeled_data(train_lab_examples, train_unlab_examples, args.batch_size, pairing_rule)
    train(args, train_loader_noshuff, train_dataset, unlab_examples_num, train_lab_examples, train_unlab_examples, valid_examples, model)
...
def train(args, train_loader_noshuff, train_dataset, unlab_examples_num, train_lab_examples, train_unlab_examples, valid_examples, model):
    """
    :param args:
    ...
    unsup_criterion = torch.nn.CrossEntropyLoss(reduction='none')
    if args.fp16:
        try:
    ...
    for epoch in train_iterator:
        ...
        feats = extract_features(train_loader_noshuff, train_lab_examples, train_unlab_examples, model)
        p_labels_data = train_dataset.label_propagation(feats, k=50, max_iter=20, alpha=0.99)
        train_unlab_examples.update_plabels(p_labels_data, args.p_cutoff)
        train_with_neg_sampling(args, model, train_lab_examples, train_unlab_examples, optimizer, scheduler, tb_writer, step_bar, skip_n_steps_in_epoch, sup_criterion, unsup_criterion)
        if epoch % (args.num_train_epochs // 4) == 0:
            # step invoke validation
            valid_examples.update_embd(model)
            valid_accuracy, valid_loss = evaluate_classification(valid_examples, model, args.batch_size_e, "evaluation/{}/runtime_eval".format(args.neg_sampling), sup_criterion)
...
def train_with_neg_sampling(args, model, train_lab_examples, train_unlab_examples, optimizer, scheduler, tb_writer, step_bar, skip_n_steps, sup_criterion, unsup_criterion):
    """
    ...
    for step in range(args.epoch_batch_num):
        ...
        weights = unlab_batch[3].to(model.device)
        ...
        sup_loss = sup_criterion(logits_lb.view(-1, 2), y_lb.view(-1))
        unsup_loss = unsup_criterion(logits_ulb.view(-1, 2), y_ulb.view(-1))
        unsup_loss = unsup_loss * weights
...
def extract_features(train_dataloader, train_lab_examples, train_unlab_examples, model):
    model.eval()
    train_iter = iter(train_dataloader)
    embeddings_all = list()
    for step in tqdm(range(len(train_dataloader)), desc='Extracting Pseudo Labels'):
        batch = next(train_iter)
        nl_ids, pl_ids, labels, islabs = batch

        lab_nl_ids = nl_ids[islabs == 1]
        lab_pl_ids = pl_ids[islabs == 1]
        y_lb = labels[islabs == 1]
        lab_batch = [lab_nl_ids, lab_pl_ids, y_lb]

        unlab_nl_ids = nl_ids[islabs == 0]
        unlab_pl_ids = pl_ids[islabs == 0]
        y_ulb = labels[islabs == 0]
        unlab_batch = [unlab_nl_ids, unlab_pl_ids, y_ulb]

        if (1 in islabs) and (0 not in islabs):
            inputs = format_batch_input(lab_batch, train_lab_examples, model)
        elif (1 not in islabs) and (0 in islabs):
            inputs = format_batch_input(unlab_batch, train_unlab_examples, model)
        else:
            assert (1 in islabs) and (0 in islabs)
            lab_inputs = format_batch_input(lab_batch, train_lab_examples, model)
            unlab_inputs = format_batch_input(unlab_batch, train_unlab_examples, model)
            inputs = dict()
            for key in lab_inputs.keys():
                inputs[key] = torch.cat([lab_inputs[key], unlab_inputs[key]], dim=0)
        with torch.no_grad():
            logits, feats = model(inputs, feature=True)
