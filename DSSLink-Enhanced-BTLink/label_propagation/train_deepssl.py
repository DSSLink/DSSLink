...
from utils import getargs, set_seed, TextDataset, Artifacts, ArtifactsDataset, UnlabArtifactsDataset
from utils import label_propagation_dataloader
import warnings
...
def train(args, train_lab_dataset, train_unlab_dataset, model, tokenizer):
    ...
    """ Train the model """
    # 根据DRNS生成的已标记软件制品对
    lab_pairs = train_lab_dataset.pair_by_DRNS()
    text_examples, code_examples = train_lab_dataset.generate_features(lab_pairs)
    ...
    # 根据日期规则生成的未标记软件制品对
    unlab_pairs = train_unlab_dataset.pair_by_datetime()
    lp_dataloader, lp_data = label_propagation_dataloader(lab_pairs, unlab_pairs, train_lab_dataset, train_unlab_dataset, tokenizer, args)
    ...
    sup_criterion = torch.nn.CrossEntropyLoss(ignore_index=-1)
    unsup_criterion = torch.nn.CrossEntropyLoss(reduction='none', ignore_index=-1)
    ...
    for idx in range(args.num_train_epochs):
        ...
        feats = extract_features(lp_dataloader, model, args)
        plabels_data = lp_data.label_propagation(feats, k=50, max_iter=20, alpha=0.99)
        ...
        if pos_plabel_num:
            ...
            unlab_artifacts_by_plabels = UnlabArtifactsDataset(text_examples, code_examples)
        ...
        for step, lab_batch in enumerate(bar):
            ...
            if pos_plabel_num:
                ...
                weights = unlab_batch[3].to(args.device)
                ...
                sup_loss = sup_criterion(logits_lb.view(-1, 2), lab_labels.view(-1))
                unsup_loss = unsup_criterion(logits_ulb.view(-1, 2), unlab_labels.view(-1))
        ...
        if results['eval_f1'] >= best_f:
            ...
            output_dir = os.path.join(output_dir, '{}'.format('model_lp.bin'))
        ...
        dfScores.to_csv(os.path.join(result_dir, args.pro + "Epoch_Metrics_lp.csv"), index=False)


def extract_features(lp_dataloader, model, args):
    model.eval()
    embeddings_all = list()
    for step, batch in enumerate(tqdm(lp_dataloader, total=len(lp_dataloader), desc='Extracting Pseudo Labels')):
        text_inputs = batch[0].to(args.device)
        code_inputs = batch[1].to(args.device)
        with torch.no_grad():
            feats = model.get_feats(text_inputs, code_inputs)
...
def test(args, model, tokenizer, stime):
    ...
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    dfScores.to_csv(os.path.join(result_dir, args.pro + "_Metrics_lp.csv"), index=False)
    ...
    df.to_csv(os.path.join(result_dir, args.pro + "_Prediction_lp.csv"), index=False)
...
def main(datasets_dir, outputs_dir, result_dir, pro, key, task_type):
    ...
    if args.do_test:
        checkpoint_prefix = args.pro + '_checkpoint-best/model_lp.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
        model = torch.load(output_dir)
        model.to(args.device)
