...
class Examples:
    ...
    def update_plabels(self, p_labels_data, p_cutoff):
        p_labels_0 = list()
        p_labels_1 = list()
        for nl_id, pl_id, sim_score, p_label, label in p_labels_data:
            if label == 0:
                p_labels_0.append(p_label)
            else:
                assert label == 1
                p_labels_1.append(p_label)
        logger.info(
            f'label_0 num: {len(p_labels_0)}, p_label_0 num: {p_labels_0.count(0)}, p_label_1 num: {p_labels_0.count(1)}')
        logger.info(
            f'label_1 num: {len(p_labels_1)}, p_label_0 num: {p_labels_1.count(0)}, p_label_1 num: {p_labels_1.count(1)}')
        ...
        logger.info('selection num: %d, selection accuracy: %.2f' % (len(correct_idx), correct_idx.mean()))
